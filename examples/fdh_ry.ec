...
game G1 = EF 
 ...
 where H = {
   ...
   if ( !in_dom(m,L) ) {
     L[m] = g^a; 
     M[i] = m; (* way of labeling messages with integers *)
 ...
 and Main = {
   ...
   (mm, sigma) = A(pk);
   h = H(mm);
   j = [0..q]; (* random j, eventually used to randomly pick the message where we have
                  the adversary "work for us" *)
...
game G2 = G1 
 ...
 where H = {
   var a : int = [0..n-1];
   if ( !in_dom(m, L) && (i <= j || in_dom(M[j], L)) ) {  (* does nothing, one step *)
     L[m] = g^a; 
     M[i] = m;
     i = i + 1;
  }
